...
export const SettingsModal = ({ defaults, onSave, onClose }: Props) => {
  const [form, setForm] = useState({
    targetMin: String(defaults.targetMin),
    targetMax: String(defaults.targetMax),
  });
  ...
  return (
    <Dialog open onClose={onClose} maxWidth="xs" fullWidth>
      <Box sx={{ px: 3, pt: 3, pb: 1, display: 'flex', alignItems: 'center', gap: 1.5 }}>
        ...
        <Box>
          <Typography sx={{ fontWeight: 900, fontSize: '1rem', color: '#0f172a' }}>デフォルト設定</Typography>
          <Typography sx={{ fontSize: '0.7rem', color: '#94a3b8' }}>全月共通の稼働時間目標</Typography>
        </Box>
      </Box>
      ...
      <DialogActions sx={{ px: 3, pb: 3, gap: 1.5 }}>
        <Button onClick={onClose} sx={{ flex: 1, borderRadius: 3, color: '#64748b', fontWeight: 700 }}>
          キャンセル
        </Button>
        <Button
          onClick={handleSave}
          variant="contained"
          sx={{ flex: 2, borderRadius: 3, bgcolor: '#0f172a', fontWeight: 700, '&:hover': { bgcolor: '#1e293b' }, boxShadow: 'none' }}
        >
          保存
        </Button>
      </DialogActions>
